...
const angel = require('./feeds/angel');
...
// feed (Yahoo) symbol -> quotes[]. One-to-MANY: a mini and its full-size parent
// track the same international contract (GOLD, GOLDM and GOLDPETAL are all GC=F),
// so a 1:1 map would leave every contract but the last one frozen at base_price.
const byYsym = new Map();
...
// `unit` converts the international price (USD per troy ounce / barrel / mmBtu /
// pound) into the Indian QUOTATION unit. Mini and micro contracts quote in the
// same unit as their full-size parent, so they share its conversion — GOLDPETAL
// is the exception, quoted per GRAM rather than per 10 g.
const COMMODITY_FEED = {
  GOLD: { y: 'GC=F', unit: 10 / OZ, quote: '₹ / 10 g' },
  GOLDM: { y: 'GC=F', unit: 10 / OZ, quote: '₹ / 10 g' },
  GOLDPETAL: { y: 'GC=F', unit: 1 / OZ, quote: '₹ / gram' },
  SILVER: { y: 'SI=F', unit: 1000 / OZ, quote: '₹ / 1 kg' },
  SILVERM: { y: 'SI=F', unit: 1000 / OZ, quote: '₹ / 1 kg' },
  SILVERMIC: { y: 'SI=F', unit: 1000 / OZ, quote: '₹ / 1 kg' },
  CRUDEOIL: { y: 'CL=F', unit: 1, quote: '₹ / barrel' },
  CRUDEOILM: { y: 'CL=F', unit: 1, quote: '₹ / barrel' },
  NATURALGAS: { y: 'NG=F', unit: 1, quote: '₹ / mmBtu' },
  NATGASMINI: { y: 'NG=F', unit: 1, quote: '₹ / mmBtu' },
...
async function initQuotes() {
  ...
  for (const inst of await all("SELECT * FROM instruments WHERE COALESCE(kind,'equity') <> 'option'")) {
    ...
    if (!byYsym.has(q.ysym)) byYsym.set(q.ysym, []);
    byYsym.get(q.ysym).push(q);
...
async function yahooTick() {
  ...
  try {
    ...
    const symbols = [...byYsym.keys()]; // deduped — minis share their parent's feed symbol
    ...
    for (const { ys, result } of results) {
      ...
      // Every contract tracking this feed symbol — GC=F drives GOLD, GOLDM and
      // GOLDPETAL, each with its own quotation-unit conversion.
      for (const q of byYsym.get(ys) || []) {
...
async function sparkTick() {
  ...
  try {
    if (needsFx()) await refreshUsdInr().catch(() => {});
    const syms = [...byYsym.keys()]; // deduped — minis share their parent's feed symbol
    const now = new Date().toISOString();
    let ok = 0;
...
// Real-time (not 15-minute-delayed) NSE equities AND genuine MCX commodity
// prices in one call, so the COMEX-derived `conv` fallback is bypassed entirely.
let angelFailures = 0;
async function angelTick() {
  if (adminPaused) return false;
  try {
    await angel.loadScripMaster();
    const nse = [], mcx = [], byToken = new Map();
    for (const q of quotes.values()) {
      if (q.kind === 'commodity') {
        const c = angel.commodityToken(q.symbol);
        if (c) { mcx.push(c.token); byToken.set(`MCX:${c.token}`, q); }
      } else {
        const e = angel.equityToken(q.symbol);
        if (e) { nse.push(e.token); byToken.set(`NSE:${e.token}`, q); }
      }
    }
    if (!nse.length && !mcx.length) throw new Error('no instruments mapped');

    const rows = await angel.quote({ NSE: nse, MCX: mcx }, 'FULL');
    const now = new Date().toISOString();
    let ok = 0;
    for (const row of rows) {
      const q = byToken.get(`${row.exchange}:${row.symbolToken}`);
      if (!q || !(row.ltp > 0)) continue;
      ok += 1;
      q.ltp = round2(row.ltp);
      q.prev_close = round2(row.close ?? q.prev_close);
      q.open = round2(row.open ?? q.open);
      q.high = round2(row.high ?? q.high);
      q.low = round2(row.low ?? q.low);
      q.bid = round2(row.depth?.buy?.[0]?.price || q.ltp * 0.9997);
      q.ask = round2(row.depth?.sell?.[0]?.price || q.ltp * 1.0003);
      q.change_pct = q.prev_close ? round2(((q.ltp - q.prev_close) / q.prev_close) * 100) : 0;
      q.volume = Math.round(row.tradeVolume || q.volume);
      q.as_of = now;
      // real MCX price — drop the "derived from international futures" caveat
      q.indicative = false;
      q.spark.push(q.ltp);
      if (q.spark.length > 90) q.spark.shift();
      recordCandle(q, Math.max(0, Math.round((row.tradeVolume || 0) / 1000)));
    }
    if (ok === 0) throw new Error('no quotes returned');
    health.lastTickAt = now;
    angelFailures = 0;
    setHealth('LIVE', `Angel One SmartAPI — ${ok} instruments, real-time`);
    emit('ALL', 'prices', snapshot());
    return true;
  } catch (e) {
    angelFailures += 1;
    if (angelFailures >= 3) setHealth('DELAYED', `SmartAPI failing: ${e.message}`);
    console.error('[angel] tick failed:', e.message);
    return false;
  }
}
...
async function watchdog() {
  ...
  // Runs every 3s and touches the DB. A transient Supabase blip must never take
  // the process down — swallow it and re-check on the next tick.
  try {
    ...
    console.error('[feed] watchdog check skipped:', e.message);
  }
}
...
async function start() {
  ...
  if (config.feedMode === 'angel') {
    // Real-time NSE + real MCX + real option chains. Falls through to Yahoo (and
    // then the simulator) so a SmartAPI outage never takes the platform down.
    if (!angel.configured()) {
      console.warn('[feed] FEED_MODE=angel but ANGEL_* credentials are missing — falling back to Yahoo.');
      config.feedMode = 'yahoo';
    } else {
      activeMode = 'angel';
      const ok = await angelTick();
      if (ok) {
        timer = setInterval(angelTick, 5000);
        timer.unref?.();
        fetchIndices();
        setInterval(fetchIndices, 15000).unref();
        console.log(`[feed] Angel One SmartAPI active — ${quotes.size} instruments, REAL-TIME (option chains live).`);
        setInterval(watchdog, 3000).unref();
        return;
      }
      console.warn('[feed] SmartAPI unreachable at startup — falling back to Yahoo (15-min delayed).');
      run('INSERT INTO feed_health_log (status, detail) VALUES (?, ?)', 'DELAYED', 'SmartAPI unreachable — using Yahoo fallback').catch(() => {});
      config.feedMode = 'yahoo';
      activeMode = 'yahoo';
    }
  }
...
// --- REAL option chain (Angel One) --------------------------------------------
// The model in options.js is a fallback ONLY. When SmartAPI is the active feed we
// cache genuine premiums / OI / IV here and price contracts off real market data.
// getQuote() must stay synchronous, so refreshes are async and write into a map
// the sync path just reads.
const _optLive = new Map();  // 'UND|YYYY-MM-DD|STRIKE|CE' -> { ltp, oi, iv, at }
const optKey = (u, e, s, t) => `${String(u).toUpperCase()}|${e}|${Number(s)}|${t}`;
const OPT_TTL = 20000;       // a cached premium older than this is considered stale

// Their underlyings ('NIFTY 50') vs SmartAPI's instrument names ('NIFTY').
const ANGEL_UNDERLYING = {
  'NIFTY 50': 'NIFTY', 'NIFTY BANK': 'BANKNIFTY', 'NIFTY FIN': 'FINNIFTY',
  'NIFTY IT': 'NIFTYIT', SENSEX: 'SENSEX',
};
const MON = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
const angelExpiryToIso = (e) => {
  const m = String(e || '').match(/^(\d{2})([A-Z]{3})(\d{4})$/);
  return m ? `${m[3]}-${String(MON.indexOf(m[2]) + 1).padStart(2, '0')}-${m[1]}` : null;
};
const isoToAngelExpiry = (iso) => {
  const m = String(iso || '').match(/^(\d{4})-(\d{2})-(\d{2})$/);
  return m ? `${m[3]}${MON[Number(m[2]) - 1]}${m[1]}` : null;
};

// Pull one underlying/expiry's chain and cache the real premiums.
async function refreshOptionChain(underlying, isoExpiry) {
  if (activeMode !== 'angel') return false;
  try {
    await angel.loadScripMaster();
    const name = ANGEL_UNDERLYING[String(underlying).toUpperCase()] || String(underlying).toUpperCase();
    const aExp = isoToAngelExpiry(isoExpiry) || angel.expiriesFor(name)[0];
    if (!aExp) return false;
    const contracts = angel.optionsFor(name, aExp);
    if (!contracts.length) return false;
    // Keep the request small: the 30 strikes nearest the spot (that's the visible chain).
    const spot = spotOf(underlying) ?? 0;
    const near = contracts
      .slice()
      .sort((a, b) => Math.abs(a.strike - spot) - Math.abs(b.strike - spot))
      .slice(0, 60);
    const rows = await angel.quote({ NFO: near.map((c) => c.token) }, 'FULL');
    const byToken = new Map(near.map((c) => [c.token, c]));
    const iso = angelExpiryToIso(aExp);
    let n = 0;
    for (const row of rows) {
      const c = byToken.get(String(row.symbolToken));
      if (!c || !(row.ltp >= 0)) continue;
      n += 1;
      _optLive.set(optKey(underlying, iso, c.strike, c.type), {
        ltp: round2(row.ltp), oi: Number(row.opnInterest || 0),
        iv: Number(row.impliedVolatility || 0), prevClose: round2(row.close || row.ltp), at: Date.now(),
      });
    }
    return n > 0;
  } catch (e) {
    console.error('[angel] option chain refresh failed:', e.message);
    return false;
  }
}

// Genuine expiry dates for an underlying (ISO), from the exchange instrument
// master. Empty unless SmartAPI is active — callers then use the computed ones.
function realExpiries(underlying) {
  if (activeMode !== 'angel') return [];
  const name = ANGEL_UNDERLYING[String(underlying).toUpperCase()] || String(underlying).toUpperCase();
  return angel.expiriesFor(name).map(angelExpiryToIso).filter(Boolean);
}

// Real premium if we have a fresh one, else null (caller falls back to the model).
function liveOption(underlying, expiry, strike, type) {
  const v = _optLive.get(optKey(underlying, expiry, strike, type));
  return v && Date.now() - v.at < OPT_TTL * 6 ? v : null;
}
...
function optionQuote(instrumentId) {
  ...
  const live = liveOption(inst.underlying, inst.expiry, inst.strike, inst.opt_type);
  // Real market premium when SmartAPI has one; otherwise the representative model.
  const ltp = live ? live.ltp : opt.premium(spot, inst.strike, inst.opt_type === 'CE', inst.expiry);
  const ref = live ? live.prevClose : (inst.base_price || ltp);
  return {
    real: !!live, open_interest: live ? live.oi : undefined, iv: live ? live.iv : undefined,
...
module.exports = {
  start, snapshot, getQuote, getCandles, getIndices, ensureDetail, feedIsLive,
  publicHealth, setAdminPaused, registerOption,
  // real option chain (Angel One); no-ops on the other feed modes
  refreshOptionChain, liveOption, realExpiries, activeFeedMode,
};
