...
async function runOnce() {
  if (busy) return;
  if (!isMarketOpen() || !market.feedIsLive()) return;
  busy = true;
  try {
    const pend = await all("SELECT o.*, i.symbol FROM orders o JOIN instruments i ON i.id = o.instrument_id WHERE o.status = 'Pending' ORDER BY o.id");
    for (const o of pend) {
