...
router.get('/markets/options', requireAuth, (req, res) => {
  ...
  const expiries = opt.upcomingExpiries();
  ...
  for (let n = -6; n <= 6; n++) {
    ...
    rows.push({
      strike,
      call: { ltp: opt.premium(spot, strike, true, expiry), oi: opt.openInterest(spot, strike), change_pct: round2((Math.random() - 0.5) * 18) },
      put: { ltp: opt.premium(spot, strike, false, expiry), oi: opt.openInterest(spot, strike), change_pct: round2((Math.random() - 0.5) * 18) },
      atm: strike === atm,
    });
  }
  res.json({ symbol, spot, atm, step, lot_size, expiry, expiries, rows, note: 'Option premiums are model-derived from the live spot (representative, not a live options feed).' });
});
