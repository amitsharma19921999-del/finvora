...
router.get('/portfolio', requireAuth, async (req, res) => {
  const holdings = await all(
    // lot_size travels with the holding so the sell ticket can size in LOTS —
    // a part-lot exit would be rejected by the exchange-lot check on the way back in.
    `SELECT h.*, i.symbol, i.name, i.exchange, i.lot_size, i.kind FROM holdings h
