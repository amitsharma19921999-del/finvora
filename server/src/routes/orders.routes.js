...
const { isMarketOpen, nextOpenLabel, segmentOf } = require('../markethours');
...
router.post('/orders', requireAuth, async (req, res) => {
  ...
  try {
    outcome = await tx(async () => {
      ...
      if (side === 'BUY') {
        ...
        const rOpen = Number((await get("SELECT IFNULL(SUM(qty * price), 0) s FROM orders WHERE user_id = ? AND side = 'BUY' AND status IN ('Pending','Executing')", req.user.id)).s);
  ...
  if (outcome.amo) {
    ...
    activity(req.user.id, 'ORDER_PLACED', `${side} ${nQty} ${inst.symbol} — after-market order, queued for ${when}`);
    notifyUser(req.user.id, 'ORDER_QUEUED', 'After-Market Order Queued',
      `Market is closed. Your ${side} order for ${nQty} × ${inst.symbol} is placed as an After-Market Order and will execute at the next market open (${when}).`, 'orders', outcome.orderId);
    notifyAdmin(side === 'BUY' ? 'BUY_ORDER' : 'SELL_ORDER', 'After-Market Order',
      `${req.user.mobile}: ${side} ${nQty} × ${inst.symbol} queued (market closed) — runs ${when}.`, 'orders', outcome.orderId);
    return res.json({ order, amo: true, message: `Market is closed — order placed as an After-Market Order. It will execute at the next market open (${when}).` });
  }
