...
// lot = the MCX contract size expressed in the QUOTED unit, so
// order value = qty x price is correct:
//   GOLD       1 kg contract, quoted per 10 g   -> 100 units
//   SILVER    30 kg contract, quoted per kg     ->  30
//   CRUDEOIL 100 bbl contract, quoted per bbl   -> 100
//   NATURALGAS 1250 mmBtu, quoted per mmBtu     -> 1250
//   COPPER   2500 kg contract, quoted per kg    -> 2500
const SEED_COMMODITIES = [
  ['GOLD', 'Gold (₹ / 10 g)', 71500, 100],
  ['SILVER', 'Silver (₹ / 1 kg)', 89000, 30],
  ['CRUDEOIL', 'Crude Oil (₹ / barrel)', 6200, 100],
  ['NATURALGAS', 'Natural Gas (₹ / mmBtu)', 250, 1250],
  ['COPPER', 'Copper (₹ / kg)', 830, 2500],
];
...
async function init() {
  ...
  for (const [symbol, name, price, lot] of SEED_COMMODITIES) {
    ...
    // Correct rows seeded before the lot sizes were verified against MCX.
    await run("UPDATE instruments SET lot_size = ?, name = ? WHERE symbol = ? AND kind = 'commodity'", lot, name, symbol);
  }
}
