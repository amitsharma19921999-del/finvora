...
async function wallet(userId) {
  ...
  const rb = Number((await get("SELECT IFNULL(SUM(qty * price), 0) s FROM orders WHERE user_id = ? AND side = 'BUY' AND status IN ('Pending','Executing')", userId)).s);
...
async function settleFill(order, px, qty, byUserId) {
  px = round2(px);
  qty = Number(qty);
  if (order.side === 'BUY') {
    await creditWallet(order.user_id, -round2(qty * px), 'BUY', 'orders', order.id, `Bought ${qty} × ${order.symbol} @ ₹${px}`);
    ...
    if (h) {
      ...
      await run('UPDATE holdings SET qty = ?, avg_price = ? WHERE id = ?', newQty, newAvg, h.id);
    } else {
      await run('INSERT INTO holdings (user_id, instrument_id, qty, avg_price) VALUES (?,?,?,?)', order.user_id, order.instrument_id, qty, px);
    }
  ...
  await creditWallet(order.user_id, proceeds, 'SELL', 'orders', order.id, `Sold ${qty} × ${order.symbol} @ ₹${px} (P&L ₹${pnl})`);
...
module.exports = {
  round2, wallet, creditWallet, sellableQty, settleFill,
