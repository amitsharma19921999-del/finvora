...
const { isMarketOpen, segmentOf, sessionSnapshot } = require('./markethours');
...
// Node kills the process on an unhandled promise rejection. On a live trading
// platform a transient DB/network blip must not take everyone offline — log it
// and keep serving. (uncaughtException still exits, so pm2 restarts cleanly.)
process.on('unhandledRejection', (e) => {
  console.error('[guard] unhandled rejection (kept running):', e && e.message ? e.message : e);
});
...
app.get('/api/health', (req, res) => res.json({
  ok: true, name: 'Finvora API', version: '2.1.0', feed: market.publicHealth(),
  market_open: isMarketOpen(),
  sessions: sessionSnapshot(),
}));

// Diagnostic: shows how the server classifies every MCX contract right now —
// its exchange/kind as stored, the segment that follows, its lot size and
// whether that segment is open. If a commodity ever gets queued as an AMO during
// MCX hours, this is the first thing to look at (no auth: nothing private here).
app.get('/api/health/segments', async (req, res) => {
  try {
    const rows = await require('./db').all(
      "SELECT id, symbol, name, exchange, kind, lot_size FROM instruments WHERE kind = 'commodity' OR exchange IN ('MCX','NCDEX') ORDER BY symbol"
    );
    res.json({
      sessions: sessionSnapshot(),
      commodities: rows.map((r) => {
        const segment = segmentOf(r);
        return { ...r, segment, open_now: isMarketOpen(new Date(), segment) };
      }),
    });
  } catch (e) {
...
// Android TWA (installed APK) verification — Digital Asset Links. Serving this lets
// the installed APK open FULL-SCREEN with no browser address bar. Put the
// assetlinks.json from your PWABuilder package at
// client/public/.well-known/assetlinks.json (then rebuild/redeploy).
app.get('/.well-known/assetlinks.json', (req, res) => {
  const candidates = [
    path.join(dist, '.well-known', 'assetlinks.json'),
    path.join(__dirname, '..', '..', 'client', 'public', '.well-known', 'assetlinks.json'),
  ];
  const f = candidates.find((p) => fs.existsSync(p));
  if (f) return res.type('application/json').sendFile(f);
...
async function main() {
  ...
  amo.startAmoRunner(); // executes queued After-Market Orders at the next market open
  require('./risk').startRiskEngine(); // per-client max-loss + timed square-off
  app.listen(config.port, () => {
    ...
    console.log(`Sessions @ ${snap.ist} IST → equity ${snap.equity.window} ${snap.equity.open ? 'OPEN' : 'closed'} | commodity ${snap.commodity.window} ${snap.commodity.open ? 'OPEN' : 'closed'}`);
    // Never print credentials on a deployed server — logs get shared/screenshotted.
    if (config.feedMode === 'sim' && !process.env.DATABASE_URL) {
      console.log('Admin login → mobile: 9999999999  password: Admin@123  (local dev only)\n');
    } else {
      console.log('Admin console → sign in with your administrator account.\n');
    }
  });
}
