...
// Auto-add RETURNING id so run().lastInsertRowid works — except on the tables
// whose primary key isn't `id` (wallets → user_id, settings → key,
// risk_profiles → user_id). Asking for `id` there is a hard SQL error.
function needsReturning(sql) {
  return /^\s*insert\s+into/i.test(sql) && !/returning/i.test(sql)
    && !/^\s*insert\s+into\s+(wallets|settings|risk_profiles)\b/i.test(sql);
}
...
-- Per-client trading limits, set by an administrator.
--   leverage        buying power multiplier (1 = cash only, 100 = 100x)
--   max_loss        auto square-off once the day's loss exceeds this many rupees
--   square_off_time 'HH:MM' IST — close open positions at this time
-- NULL max_loss / square_off_time means that control is off for the client.
CREATE TABLE IF NOT EXISTS risk_profiles (
  user_id INTEGER PRIMARY KEY REFERENCES users(id),
  leverage DOUBLE PRECISION NOT NULL DEFAULT 1,
  max_loss DOUBLE PRECISION,
  square_off_time TEXT,
...
CREATE TABLE IF NOT EXISTS holdings (
  ...
  -- Cash actually blocked for this position. At 1x it equals the full cost; on a
  -- leveraged buy only cost/leverage leaves the wallet, and this is what gets
  -- released (plus or minus P&L) when the position is closed.
  margin_used DOUBLE PRECISION,
  UNIQUE(user_id, instrument_id)
);
ALTER TABLE holdings ADD COLUMN IF NOT EXISTS margin_used DOUBLE PRECISION;
...
// MCX-segment commodity contracts. Symbols must match both COMMODITY_FEED in
// marketdata.js and the `name` field of MCX rows in the Angel scrip master.
// [symbol, display name, cold-start placeholder price, lot_size]
//
// lot_size = the MCX contract size EXPRESSED IN THE QUOTED UNIT, so that
// `order value = qty x live price` is the real notional of one contract.
//
// This is NOT the "Lot size" column brokers print — that is the physical trading
// unit ("1 kg", "2.5 MT") — and it is NOT what a broker API hands you:
//   * Angel's scrip master returns the trading unit with the unit dropped:
//     GOLD 1 (=1 kg), GOLDM 100 (=100 g), ZINC 5 (=5 MT). Meaningless as a number.
//   * Kite returns lot_size = 1 for EVERY MCX instrument, by design.
// Seeding this column from either source understates GOLD by 100x and NATURALGAS
// by 1250x. The multipliers below come from the MCX contract specifications and
// are unit-tested in test/lot-sizes.js — re-verify after any MCX circular titled
// "Modification in the Contract Specification of ...".
//
//   symbol      trading unit   quoted per   multiplier      ~1 lot (Sep 2026)
//   GOLD        1 kg           10 g         1000/10 = 100   ~₹1.53 crore
//   GOLDM       100 g          10 g          100/10 =  10   ~₹15.3 lakh
//   GOLDPETAL   1 g            1 g (!)                 1    ~₹15,400
//   SILVER      30 kg          1 kg                   30    ~₹71 lakh
//   SILVERM     5 kg           1 kg                    5    ~₹12 lakh
//   SILVERMIC   1 kg           1 kg                    1    ~₹2.4 lakh
//   CRUDEOIL    100 bbl        1 bbl                 100    ~₹8 lakh
//   CRUDEOILM   10 bbl         1 bbl                  10    ~₹80,000
//   NATURALGAS  1250 mmBtu     1 mmBtu              1250    ~₹3.5 lakh
//   NATGASMINI  250 mmBtu      1 mmBtu               250    ~₹70,000
//   COPPER      2500 kg        1 kg                 2500    ~₹34 lakh
//
// Quotation units are NOT uniform inside a family: GOLDPETAL is quoted per GRAM
// while the rest of the gold family is per 10 g. Never share a unit per underlying.
// MCX lists no copper mini/micro, so COPPER has no retail-sized contract.
const SEED_COMMODITIES = [
  // full-size contracts
  ['GOLD', 'Gold — 1 kg lot (₹ / 10 g)', 152815, 100],
  ['SILVER', 'Silver — 30 kg lot (₹ / kg)', 237500, 30],
  ['CRUDEOIL', 'Crude Oil — 100 barrel lot (₹ / barrel)', 8200, 100],
  ['NATURALGAS', 'Natural Gas — 1250 mmBtu lot (₹ / mmBtu)', 281.2, 1250],
  ['COPPER', 'Copper — 2500 kg lot (₹ / kg)', 1378.1, 2500],
  // retail-sized contracts — the full-size lots above are ₹8 lakh to ₹1.5 crore
  // of notional, which no ordinary client can carry. These are the same contracts
  // Groww/Zerodha clients actually trade.
  ['GOLDM', 'Gold Mini — 100 g lot (₹ / 10 g)', 152815, 10],
  ['GOLDPETAL', 'Gold Petal — 1 g lot (₹ / gram)', 15356, 1],
  ['SILVERM', 'Silver Mini — 5 kg lot (₹ / kg)', 237500, 5],
  ['SILVERMIC', 'Silver Micro — 1 kg lot (₹ / kg)', 237500, 1],
  ['CRUDEOILM', 'Crude Oil Mini — 10 barrel lot (₹ / barrel)', 8200, 10],
  ['NATGASMINI', 'Natural Gas Mini — 250 mmBtu lot (₹ / mmBtu)', 281.2, 250],
];
...
async function init() {
  if (process.env.DATABASE_URL) {
    ...
    const pool = new Pool({
      ...
      max: 8,
      keepAlive: true,               // stop the pooler silently dropping idle sockets
      idleTimeoutMillis: 30000,      // recycle our idle clients before Supabase does
      connectionTimeoutMillis: 10000,
    });
    // Supabase's connection pooler closes idle connections. node-postgres raises
    // that as an 'error' EVENT on the pool — and an unhandled 'error' event kills
    // the whole process (which is why pm2 was restart-looping). Listening here
    // makes it a logged, recovered event: the pool just opens a fresh client.
    pool.on('error', (e) => console.error('[db] idle client dropped (recovered):', e.message));
    backend = {
    ...
    // No DATABASE_URL -> the embedded local DB. That is correct for local dev, but
    // on a deployed server it means the app silently starts on an EMPTY database:
    // real customers cannot log in and their holdings look gone. Make it loud.
    if (process.env.NODE_ENV === 'production' || process.env.PM2_HOME || process.env.PM_ID) {
      console.error('='.repeat(72));
      console.error('!! DATABASE_URL IS NOT SET — starting on the LOCAL, EMPTY database.');
      console.error('!! Live customer accounts, funds and holdings will NOT be visible.');
      console.error('!! Set DATABASE_URL in server/.env and restart before serving users.');
      console.error('='.repeat(72));
    }
  ...
  for (const [symbol, name, price, lot] of SEED_COMMODITIES) {
    ...
    // Repair rows seeded before the lot sizes were verified against MCX.
    // NOTE: deliberately NOT filtered on kind='commodity'. A row created by an
    // earlier seed (or by hand) as kind='equity'/exchange='NSE' would survive the
    // ON CONFLICT DO NOTHING above, and a filtered UPDATE could never fix it — so
    // it would keep the 15:30 equity cut-off and every MCX order placed in the
    // evening session was queued as an AMO instead of executing.
    await run(
      "UPDATE instruments SET lot_size = ?, name = ?, base_price = ?, exchange = 'MCX', kind = 'commodity' WHERE symbol = ?",
      lot, name, price, symbol,
    );
  }
}
