...
const { settleFill, round2, activity, notifyUser, reservedForOrders } = require('./util');
      ...
      try {
        result = await tx(async () => {
          if (o.side === 'BUY') {
            ...
            // available excluding THIS order's own reservation (margin, not notional)
            const rOpen = await reservedForOrders(o.user_id, o.id);
            ...
            const levNow = Number((await get('SELECT leverage FROM risk_profiles WHERE user_id = ?', o.user_id) || {}).leverage) || 1;
            const available = round2(round2((w ? w.balance : 0) - rOpen - rw) * levNow);
          ...
          // honour the client's leverage when the queued order finally fills
          const lev = Number((await get('SELECT leverage FROM risk_profiles WHERE user_id = ?', o.user_id) || {}).leverage) || 1;
          await settleFill(o, px, o.qty, null, lev); // sets Executed, price = px
          return 'executed';
        });
