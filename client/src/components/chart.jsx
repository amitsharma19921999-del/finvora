...
// '#rrggbb' | '#rgb' | 'rgb(...)' -> 'rgba(r, g, b, a)'. Lets the volume bars reuse
// the themed up/down colours instead of hardcoded ones.
function withAlpha(color, a) {
  ...
  if (rgb) { const p = rgb[1].split(',').map((v) => v.trim()); return `rgba(${p[0]}, ${p[1]}, ${p[2]}, ${a})`; }
  return s;
}
const volColors = (c) => [withAlpha(c.up, 0.5), withAlpha(c.down, 0.5)];

// Read the current theme's chart colours from CSS variables (re-read on theme switch).
// accent uses --accent-strong so the line/area colour stays readable in light mode.
function readColors() {
  return {
    ...
    accent: cssVar('--accent-strong', '#2dd4bf'),
  };
}
...
export function ProChart({ candles, resetKey, symbol, exchange = 'NSE', chartType = 'candles', height = 380 }) {
  ...
  const makePriceSeries = (chart, type) => {
    if (type === 'line') {
      return chart.addLineSeries({ color: colorsRef.current.accent, lineWidth: 2, priceLineVisible: true, lastValueVisible: true });
    }
    if (type === 'area') {
      return chart.addAreaSeries({ lineColor: colorsRef.current.accent, topColor: 'rgba(45,212,191,.25)', bottomColor: 'rgba(45,212,191,0)', lineWidth: 2 });
    }
    return chart.addCandlestickSeries({
      upColor: colorsRef.current.up, downColor: colorsRef.current.down, borderVisible: false,
      wickUpColor: colorsRef.current.up, wickDownColor: colorsRef.current.down,
    });
  };
  ...
  useEffect(() => {
    const chart = createChart(wrapRef.current, {
      ...
      layout: { background: { type: ColorType.Solid, color: 'transparent' }, textColor: colorsRef.current.text2, fontSize: 11, fontFamily: 'inherit', attributionLogo: false },
      grid: { vertLines: { color: colorsRef.current.grid }, horzLines: { color: colorsRef.current.grid } },
      crosshair: {
        mode: CrosshairMode.Normal,
        vertLine: { color: colorsRef.current.text3, width: 1, style: LineStyle.Dashed, labelBackgroundColor: colorsRef.current.border },
        horzLine: { color: colorsRef.current.text3, width: 1, style: LineStyle.Dashed, labelBackgroundColor: colorsRef.current.border },
      },
      rightPriceScale: { borderColor: colorsRef.current.border, scaleMargins: { top: 0.08, bottom: 0.26 } },
      timeScale: { borderColor: colorsRef.current.border, timeVisible: true, secondsVisible: false, rightOffset: 4, barSpacing: 8 },
  ...
  // Re-theme the chart live when the app switches light/dark.
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (!chart) return;
      chart.applyOptions({
        layout: { textColor: c.text2 },
        grid: { vertLines: { color: c.grid }, horzLines: { color: c.grid } },
        crosshair: {
          vertLine: { color: c.text3, labelBackgroundColor: c.border },
          horzLine: { color: c.text3, labelBackgroundColor: c.border },
        },
      ...
      if (p) {
        if (typeRef.current === 'candles') p.applyOptions({ upColor: c.up, downColor: c.down, wickUpColor: c.up, wickDownColor: c.down });
        else if (typeRef.current === 'line') p.applyOptions({ color: c.accent });
        else if (typeRef.current === 'area') p.applyOptions({ lineColor: c.accent });
      }
      // volume bar colours live on the data, so re-feed them in the new theme
      const v = volRef.current;
      if (v && dataRef.current.length) {
        const [vUp, vDown] = volColors(c);
        v.setData(dataRef.current.map((bar) => toVol(bar, vUp, vDown)));
      }
    };
    const mo = new MutationObserver((muts) => { if (muts.some((m) => m.attributeName === 'data-theme')) apply(); });
  ...
  useEffect(() => {
    ...
    const [vUp, vDown] = volColors(colorsRef.current);
    const vols = candles.map((c) => toVol(c, vUp, vDown));
