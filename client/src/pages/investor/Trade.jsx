...
export default function Trade() {
  ...
  useEffect(() => {
    if (!selectedId && quotes.length) { setSelectedId(quotes[0].instrument_id); setPrice(px2(quotes[0].ltp)); }
  }, [quotes, selectedId]);
  ...
  // Orders execute at the current market price — quantity is the only input.
  const marketPrice = sel ? sel.ltp : 0;
  const est = (Number(qty) || 0) * marketPrice;
  ...
  const submit = async (e) => {
    ...
    if (!Number.isInteger(nQty) || nQty <= 0) { setQtyError('Quantity must be a positive whole number.'); return; }
    setBusy(true);
    try {
      // No price sent — the server fixes it to the live market price.
      const res = await api.post('/api/orders', { instrument_id: sel.instrument_id, side, qty: nQty });
              ...
              <form onSubmit={submit} className="ticket-body">
                ...
                <div className="row-between small tk-avail">
                  {side === 'BUY'
                    ...
                    : <><span className="text-muted">Holding</span><strong className="mono">{portfolio ? `${num(heldQty)} qty` : '—'}</strong></>}
                </div>

                <Field label="Quantity" required error={qtyError}>
                  <input className="input" type="number" min="1" step="1" inputMode="numeric" placeholder="Qty" value={qty}
        ...
        {bottomTab === 'orders' ? (
          ...
          !portfolio ? <div className="pad"><Spinner label="Loading holdings…" /></div>
            ...
            : (
              <div className="tbl-scroll">
                <table className="term-table">
                  <thead><tr><th>Instrument</th><th className="ta-r">Qty</th><th className="ta-r">Avg</th><th className="ta-r">LTP</th><th className="ta-r">Value</th><th className="ta-r">P&L</th></tr></thead>
                  <tbody>
