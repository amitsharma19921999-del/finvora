...
export default function InvestorDetail() {
  ...
  // Per-client trading limits: leverage, max-loss auto square-off, timed square-off.
  const [riskOpen, setRiskOpen] = useState(false);
  const [riskLev, setRiskLev] = useState('1');
  const [riskMaxLoss, setRiskMaxLoss] = useState('');
  const [riskTime, setRiskTime] = useState('');
  const [riskPnl, setRiskPnl] = useState(null);

  const openRisk = async () => {
    setBusy(true);
    try {
      const d = await api.get(`/api/admin/investors/${id}/risk`);
      setRiskLev(String(d.risk.leverage ?? 1));
      setRiskMaxLoss(d.risk.max_loss == null ? '' : String(d.risk.max_loss));
      setRiskTime(d.risk.square_off_time || '');
      setRiskPnl(d.day_pnl);
      setRiskOpen(true);
  ...
  const saveRisk = async () => {
    setBusy(true);
    try {
      const d = await api.post(`/api/admin/investors/${id}/risk`, {
        leverage: riskLev, max_loss: riskMaxLoss, square_off_time: riskTime,
      });
      toast.success(d.message);
      setRiskOpen(false);
    } catch (e) { toast.error(e.message); } finally { setBusy(false); }
  ...
  const squareOffNow = async () => {
    setBusy(true);
    try {
      const d = await api.post(`/api/admin/investors/${id}/square-off`, {});
      toast.success(d.message);
      await load();
    } catch (e) { toast.error(e.message); } finally { setBusy(false); }
  };
    ...
    <div>
      ...
      <div className="row-between mt-2" style={{ marginBottom: 18 }}>
        ...
        <div className="row" style={{ gap: 10 }}>
          ...
          <button type="button" className="btn btn-sm" disabled={busy} onClick={openRisk}>
            Trading limits
          </button>
        </div>
      </div>
      ...
      <Modal
        open={riskOpen}
        onClose={() => !busy && setRiskOpen(false)}
        title={`Trading limits — ${displayName}`}
        footer={(
          <>
            <button type="button" className="btn btn-ghost" onClick={() => setRiskOpen(false)} disabled={busy}>Cancel</button>
            <button type="button" className="btn btn-primary" onClick={saveRisk} disabled={busy}>
      ...
      >
        <div className="row-between small">
          ...
          <b className="mono">{inr(wallet.available)}</b>
        </div>
        {riskPnl !== null && (
          <div className="row-between small">
            <span className="text-muted">Today&apos;s P&amp;L</span>
            <b className={`mono ${riskPnl >= 0 ? 'text-up' : 'text-down'}`}>{inr(riskPnl)}</b>
          </div>
        )}

        <label className="field mt-2">
          <span className="field-label">Leverage (buying power multiplier)</span>
          <input className="input mono" type="number" min="1" max="500" step="1" value={riskLev} disabled={busy}
            onChange={(e) => setRiskLev(e.target.value)} placeholder="1" />
          <span className="field-hint">
            Buying power ≈ <b className="mono">{inr((wallet.available || 0) * (Number(riskLev) || 1))}</b>
            {Number(riskLev) > 1 ? ` (${inr(wallet.available)} × ${Number(riskLev)}x)` : ' — cash only'}
          </span>
        </label>

        <label className="field">
          <span className="field-label">Auto square-off past a loss of (₹)</span>
          <input className="input mono" type="number" min="1" step="1" value={riskMaxLoss} disabled={busy}
            onChange={(e) => setRiskMaxLoss(e.target.value)} placeholder="e.g. 12000 — blank for no limit" />
          <span className="field-hint">
            When the day&apos;s loss (realised + open) passes this, every position is closed at market.
          </span>
        </label>

        <label className="field">
          <span className="field-label">Daily square-off time (IST)</span>
          <input className="input mono" type="text" value={riskTime} disabled={busy}
            onChange={(e) => setRiskTime(e.target.value)} placeholder="e.g. 15:15 — blank for none" />
          <span className="field-hint">Open positions are closed at this time each trading day.</span>
        </label>

        {Number(riskLev) > 5 && (
          <div className="banner banner-warn small">
            <div>
              At <b>{Number(riskLev)}x</b>, a move of about{' '}
              <b>{(100 / Number(riskLev)).toFixed(2)}%</b> against the position wipes out the full
              deposit. SEBI-registered brokers are capped near 5x intraday.
            </div>
          </div>
        )}

        <button type="button" className="btn btn-danger btn-block mt-2" disabled={busy} onClick={squareOffNow}>
