...
export default function OrdersQueue() {
  ...
  // Delete… modal — removes the trade and reverses its effect on funds/holdings
  const [delOrder, setDelOrder] = useState(null);
  const [delReason, setDelReason] = useState('');
  const [delBusy, setDelBusy] = useState(false);
  const [delReverse, setDelReverse] = useState(false); // opt-in: also undo funds/holdings

  const doDelete = async () => {
    if (delReason.trim().length < 3) { toast.error('Enter a reason — it is stored in the audit log.'); return; }
    setDelBusy(true);
    try {
      const d = await api.post(`/api/admin/orders/${delOrder.id}/delete`, { reason: delReason.trim(), reverse: delReverse });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (o) => {
        ...
        // A settled order can still be removed — the server reverses the money and
        // holdings first, so deleting can never leave the books out of balance.
        if (o.status !== 'Pending' && o.status !== 'Executing') {
          return (
            <button type="button" className="btn btn-danger btn-sm tap" disabled={busy}
              onClick={() => { setDelReason(''); setDelReverse(false); setDelOrder(o); }}>
              Delete…
            </button>
          );
        }
  ...
  return (
    <div>
      ...
      <Modal
        open={!!delOrder}
        onClose={() => !delBusy && setDelOrder(null)}
        title="Delete trade"
        footer={delOrder && (
          <>
            <button type="button" className="btn btn-ghost" disabled={delBusy} onClick={() => setDelOrder(null)}>Back</button>
            <button type="button" className="btn btn-danger" disabled={delBusy || delReason.trim().length < 3} onClick={doDelete}>
        ...
        {delOrder && (
          <div>
            <div className="banner banner-warn small">
              This permanently removes <b>{delOrder.side} {num(delOrder.qty)} × {delOrder.symbol}</b> for
              <span className="mono"> {delOrder.mobile}</span> from the database, along with its ledger
              entries. It disappears from the investor&apos;s Orders and History straight away.
            </div>

            {delOrder.status === 'Executed' && (
              <label className="row mt-2" style={{ gap: 8, alignItems: 'flex-start', cursor: 'pointer' }}>
                <input type="checkbox" checked={delReverse} disabled={delBusy}
                  onChange={(e) => setDelReverse(e.target.checked)} style={{ marginTop: 3 }} />
                <span className="small">
                  <b>Also reverse the money and holdings</b><br />
                  <span className="text-muted">
                    {delReverse
                      ? (delOrder.side === 'BUY'
                        ? 'The shares will be removed and the money refunded to the wallet.'
                        : 'The sale proceeds will be taken back and the shares restored.')
                      : `Leave unticked to delete the record only — the wallet balance and holdings stay exactly as they are.`}
                  </span>
                </span>
              </label>
            )}
            ...
            <p className="small text-muted mt-2">
              This cannot be undone. The investor is notified, and the action is recorded in the audit log
              with your reason.
            </p>
            <Field label="Reason (audit log)" required>
              <textarea className="input" rows="3" value={delReason} disabled={delBusy}
                placeholder="e.g. Duplicate order created by a double tap — removed at the investor's request."
